...
#include <fstream>
#define FILENAME "ClubB.txt" 


ClubB::ClubB(int type, int id, string name, int cRank, string m_managerAccount, string m_managerPassword)
{
	this->m_Type = type;
	this->m_Id = id;
	...
	//cout << "B会员人数为：" << this->m_memNum << endl;
...
void ClubB::showInfo()
{
	...
	cout << "类型：B" << " "
		...
		<< "\t俱乐部：" << this->m_ClubName << " "
		<< "\t排名：" << this->m_ClubRank << " "
		<< "\n特点：" << " B级俱乐部：商业综合型健身房俱乐部  ->  1.设施齐全 2.课程丰富" << endl;
}
...
void ClubB::returnBack(bool& judge)
{
	cout << "正在返回......" << endl;
	judge = false;
	system("pause");
...
int ClubB::IsAccountExist(string account)
{
	int index = -1;
...
bool ClubB::adminLogin(const string& account, const string& password)
{
...
bool ClubB::userLogin(const string& account, const string& password)
{
...
void ClubB::renew_Sin(int index)
{
...
void  ClubB::processConsumption(int index)
{
...
void  ClubB::cancelCard(int index)
{
	if (this->m_memArray.at(index)->m_cardFileIsEmpty)
	{
		cout << this->m_memArray.at(index)->filename << "文件不存在或记录为空" << endl;
